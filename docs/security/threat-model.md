...
- First assessment — 22 threats identified across 5 components.

---

## Management Summary

This assessment of **appsec-plugin** (v0.9.0-beta) identified **4 critical and 8 high-severity vulnerabilities** across the plugin's five analyzed components. The plugin is a developer security tool that processes source code from arbitrary repositories and transmits it to the Anthropic API. The most severe issue is that `run-headless.sh` constructs a shell command using `eval "$CLAUDE_CMD"` where the PROMPT string is assembled from unsanitized user-supplied `--repo`, `--output`, and `--category` arguments, enabling shell injection for any script consumer who passes attacker-controlled paths. Additionally, the external REST context endpoint (`rest_url` in `config.json`) accepts any URL including `http://` scheme without origin restriction, creating an SSRF vector. The plugin's `agent_logger.py` writes to a hardcoded path `docs/security/.hook-events.log` derived from `os.getcwd()` rather than the configured `OUTPUT_DIR`, creating a log-path divergence that can cause logs to appear in unintended directories. These deficiencies collectively mean the plugin **should not be invoked from automated CI/CD scripts with untrusted repository paths** until the P1 issues are resolved.

### Structural Defects

The following structural defects are architectural — they cannot be fixed by patching a single file and are the root cause behind most individual threats below.

- **Shell injection via `eval` in run-headless.sh** — the `PROMPT` string is assembled from unsanitized user-controlled arguments and passed to `eval "$CLAUDE_CMD"` at line 354, allowing shell metacharacter injection via `--repo`, `--output`, and `--stride-model` arguments.
- **SSRF via unrestricted URL in context endpoint** — `rest_url` and `REQUIREMENTS_URL_OVERRIDE` accept any URL including `file://`, `gopher://`, and internal network addresses at runtime without allowlist enforcement, though `validate_config.py` does check for `http://` or `https://` scheme at configuration time only.
- **Hardcoded log path ignoring OUTPUT_DIR** — `agent_logger.py` unconditionally writes logs to `os.getcwd()/docs/security/.hook-events.log` regardless of the configured `OUTPUT_DIR`, creating security-sensitive log data in unexpected locations when the `--output` flag is used.
- **No input sanitization in hook scripts** — `security_steering.py` reads from stdin and `agent_logger.py` processes hook events without sanitizing newlines or special characters in log output (though `_clip()` replaces newlines).
- **REPO_ID unquoted in curl JSON body** — in the context-resolver's `curl -d "{\"repo_url\": \"$REPO_ID\"}"`, if `REPO_ID` contains double-quotes or backslashes (e.g., from a pathological git remote URL), the JSON becomes malformed.

### Risk Distribution

| Severity | Count | Findings |
|----------|-------|----------|
| Critical | 4 | Shell injection, SSRF, insecure `eval`, log path traversal |
| High | 8 | Log injection, REPO_ID injection, secret pattern gaps, unvalidated URL at runtime |
| Medium | 7 | Weak error handling, missing input bounds, dependency freshness |
| Low | 3 | Hardening gaps |

<!-- QA: Management Summary is missing the "Top Critical Findings" sub-section — add a brief list of the 3-5 most critical findings before the Immediate Actions table -->
...
🟡 **Needs Improvement** — The plugin has a solid security intent and its Python validation scripts are well-structured. However, four critical issues exist in the operational surface: shell injection via `eval`, SSRF via unchecked URLs at runtime, hardcoded log path, and JSON injection. These are all easily fixable (Low/Medium effort) and should be resolved before the plugin is used in any CI/CD automation with untrusted inputs.

### Operational Strengths

<!-- QA: Management Summary is missing the "Operational Strengths" sub-section — add 3-5 bullets summarizing what security controls are working well (e.g., schema validation, secret masking, lock files, TLS enforcement) -->
...
|---------|---------|-------|
| API Gateway | ❌ | No gateway; Claude Code skill system is the entry point |
| BFF (Backend-for-Frontend) | N/A | No frontend SPA |
| Defense-in-depth | ⚠️ Partial | Lock file, schema validation, secret masking — but no input sanitization in shell scripts |
| Separation of concerns | ✅ | Agents, scripts, skills, and hooks are cleanly separated |
| Least-privilege | ⚠️ Partial | `settings.json` allowlists Bash commands but `python3:*` is unrestricted |
| Secrets management | ✅ | No hardcoded secrets in config.json; ANTHROPIC_API_KEY via env var only |
| Network segmentation | ✅ | No listening ports; curl calls use TLS |
| Secure defaults | ⚠️ Partial | `external_context.enabled: true` but `rest_url: null`; requires URL to activate |

#### Trust Model Evaluation

The plugin operates as a local developer tool with no network-facing service. Trust flows are: (1) developer invokes the plugin via Claude Code or `run-headless.sh`; (2) the plugin reads local repository files; (3) the orchestrator agent calls the Anthropic API; (4) optionally, the context-resolver calls an external REST endpoint. The model is **fail-open** in one critical way: if the external context endpoint is unreachable, the assessment continues without it (acceptable), but if the endpoint URL is attacker-controlled (via SSRF), it could exfiltrate the repo URL or receive attacker-injected context. There is **unnecessary transitivity** in `run-headless.sh`: the user's file-system path arguments flow unsanitized into a shell-eval'd command string.

#### Authentication and Authorization Architecture

The plugin has no authentication or authorization of its own — it relies entirely on the developer's local access controls and Claude Code's authentication. The Anthropic API authentication is handled transparently by Claude Code (subscription) or via the `ANTHROPIC_API_KEY` environment variable (API billing). No token lifecycle, session management, or MFA concern applies. The external REST context endpoint has no authentication requirements in the plugin's design — any URL that responds with JSON is accepted.

#### Key Architectural Risks

| # | Structural Risk | Impact if Exploited | Linked Threats |
|---|-----------------|---------------------|----------------|
| 1 | `eval` in run-headless.sh with user-controlled args | Full shell command execution as the invoking user | [T-001](#t-001) |
| 2 | No URL allowlist for external context endpoint | SSRF to internal network or exfiltration of repo URL | [T-002](#t-002), [T-003](#t-003) |
| 3 | Hardcoded log path in agent_logger.py | Logs written to wrong directory; potential path confusion | [T-004](#t-004) |
| 4 | Unvalidated REPO_ID in curl JSON body | JSON injection / malformed requests | [T-005](#t-005) |
| 5 | `python3:*` unrestricted in settings.json | Any Python script can be executed by agents | [T-015](#t-015) |

#### Cross-Cutting Architecture Findings

The following six themes synthesize the structural security concerns observed across the code base. Individual code-level instances appear as threats in Section 8; this sub-section explains the underlying architectural pattern that produces them.
...
### 3.1 CI/CD Headless Invocation with Shell Injection

This flow shows how a developer invokes `run-headless.sh` in CI/CD, and the shell injection attack path that exists when `--repo` contains shell metacharacters.
...
sequenceDiagram
    ...
    alt Normal execution — trusted path %% attack-path
...
### 3.2 External Context Endpoint SSRF

This flow shows the context-resolver fetching external context, and the SSRF attack path when `rest_url` points to an internal network address.
...
sequenceDiagram
    ...
    else SSRF attack — internal URL %% attack-path
...
### 3.3 Hook Script Log Path — CWD vs OUTPUT_DIR

This flow shows how `agent_logger.py` determines the log file path, and the misalignment that occurs when `--output` points outside the repository.
...
sequenceDiagram
    ...
    alt Expected: logs in OUTPUT_DIR %% attack-path
...
### 3.4 Security Steering Hook — Prompt Injection Boundary

This flow shows how `security_steering.py` processes incoming prompts, and the boundary between triggering and non-triggering patterns.
...
sequenceDiagram
    ...
    participant Steering as security_steering.py

    User->>Claude: "help me write a function"
    Claude->>Steering: UserPromptSubmit {"prompt": "help me write a function"}
    Steering->>Steering: count strong=0, code=1(function), action=1(write)

    alt Trigger condition: code_action (code>=1 AND action>=1)
        User->>Claude: "write a function to handle auth tokens"
        Claude->>Steering: {"prompt": "write a function to handle auth tokens"}
        Steering->>Steering: code_action_code_min=1 matched, action_min=1 matched
        Steering-->>Claude: inject "Security steering active. Always implement secure-by-default..."
    else Below threshold — no injection %% attack-path
        Steering-->>Claude: {}
        Note over Steering: No security context injected for borderline prompts
    end
```

**Key takeaway:** The security steering hook correctly identifies security-sensitive prompts and injects guidance, but the keyword lists are loaded from a JSON file that any user with filesystem access can modify to disable injection by removing keywords or raising thresholds.
...
These findings combine high exploitability with maximum impact. Every entry here also appears in Section 9.
...
<!-- QA: T-002 risk corrected in YAML from "High" to "Critical" to match MD -->
...
<!-- QA: T-003 risk corrected in YAML from "High" to "Critical" to match MD -->
...
<!-- QA: T-004 risk corrected in YAML from "High" to "Critical" to match MD -->
...
| ID | Component | STRIDE | Threat Scenario | Likelihood | Impact | Risk | Controls in Place | Mitigations |
|----|-----------|--------|-----------------|------------|--------|------|-------------------|-------------|
<!-- QA: T-005 risk corrected in YAML from "Medium" to "High" to match MD -->
...
<!-- QA: T-006 risk corrected in YAML from "Low" to "High" to match MD -->
...
<!-- QA: T-008 risk corrected in YAML from "Low" to "High" to match MD -->
...
<!-- QA: T-010 has no mitigation assigned — add an M-NNN entry in Section 10 if this medium risk gap needs a remediation -->
<!-- QA: T-010 risk corrected in YAML from "High" to "Medium" to match MD -->
...
<!-- QA: T-011 risk corrected in YAML from "Low" to "High" to match MD -->
...
<!-- QA: T-012 risk corrected in YAML from "Low" to "High" to match MD -->
...
<!-- QA: T-014 risk corrected in YAML from "Medium" to "High" to match MD -->
...
| ID | Component | STRIDE | Threat Scenario | Likelihood | Impact | Risk | Controls in Place | Mitigations |
|----|-----------|--------|-----------------|------------|--------|------|-------------------|-------------|
<!-- QA: T-020 risk corrected in YAML from "Medium" to "Low" to match MD -->
...
<!-- QA: T-021 risk corrected in YAML from "Medium" to "Low" to match MD -->
...
<!-- QA: T-022 risk corrected in YAML from "Medium" to "Low" to match MD -->
...
The following chain shows how the Critical findings combine into a single attacker workflow. Each node links directly to its full detail row in Section 8.1.

<!-- QA: diagram uses LR layout — consider switching to TD for readability -->
```mermaid
graph LR
    classDef crit fill:#FFB6C1,stroke:#c00,color:#000,stroke-width:2px
    Start(["CI/CD runner\nwith attacker-controlled\n--repo arg"]):::crit
    T1["T-001\nShell injection\nvia eval"]:::crit
    T2["T-002\nSSRF via\ncontext endpoint"]:::crit
    T3["T-003\nSSRF via\n--requirements URL"]:::crit
    T4["T-004\nLog data leaked\nto wrong directory"]:::crit
    Goal1(["Arbitrary code\nexecution as CI user"]):::crit
    Goal2(["Internal network\nprobed / creds leaked"]):::crit

    Start -->|"--repo with metachar"| T1
    T1 -->|"eval executes"| Goal1
    Start -->|"config.json modified"| T2
    Start -->|"--requirements flag"| T3
    T2 -->|"curl to internal URL"| Goal2
    T3 -->|"curl to file:// URL"| Goal2
    T4 -->|"logs in analyzed repo"| Goal2
```
...
**Priority:** **P1 — Immediate** · **Severity:** 🟠 High · **Effort:** Low
...
**Priority:** **P2 — This Sprint** · **Severity:** 🟠 High · **Effort:** Low
...
**Why:** The external context endpoint accepts any `http://` URL. An HTTP connection is vulnerable to network MITM, allowing injection of adversarial context. Requiring `https://` and documenting that the endpoint must be authenticated reduces the attack surface.
...
**Priority:** **P3 — Next Quarter** · **Severity:** 🟡 Medium · **Effort:** Medium
