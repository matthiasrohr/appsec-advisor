...
1. **Adapt the reference baseline.** Copy `data/appsec-requirements-fallback.yaml` (53 requirements across 10 categories) and rewrite the IDs and text to match your organisation. Serve it over HTTP (dev: `python3 scripts/mock-context-server.py`) or commit it to a Git-hosted raw URL.
