...
| File | Change |
|------|--------|
| `agents/phases/phase-group-finalization.md` | Replace Run Statistics appendix spec (lines 454-543) with enhanced 6-subsection template; add `run_statistics` to YAML schema (lines 28-31) |
| `agents/appsec-qa-reviewer.md` | Update Check 12c patching instructions (lines 989-1019) to target new table structures and new YAML fields |
| `skills/create-threat-model/SKILL.md` | Update console summary format (lines 530-623) with token breakdown, multi-model costs, agent roster |
