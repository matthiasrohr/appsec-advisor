...
```bash
python3 plugin/scripts/validate_fragment.py verdict "$OUTPUT_DIR/.fragments/ms-verdict.json"
```
...
4. `category_taxonomy` (aus `plugin/data/threat-category-taxonomy.yaml`) für `TH-NN` → `title`.
...
```json
// plugin/schemas/fragments/incident-response.schema.json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
...
```jinja
{# plugin/templates/fragments/incident-response.md.j2 #}
...
**Contract**: `plugin/data/sections-contract.yaml` — die deklarative Struktur-Spezifikation des rendered Dokuments.

**Schema**: eine JSON-Schema-Datei in `plugin/schemas/fragments/` — bindet LLM-geschriebene Daten-Fragmente.
...
**Renderer**: `plugin/scripts/compose_threat_model.py` — das deterministische Python-Script, das den finalen Markdown aus Contract + Fragments + yaml komponiert.
...
**QA Agent**: `plugin/scripts/qa_checks.py` — der Post-Render-Validator mit Subcommands `links`, `xrefs`, `anchors`, `invariants`, `ms_structure`, `contract`, `all`.
...
**TH-NN**: Threat-Category-ID aus `plugin/data/threat-category-taxonomy.yaml`. Architekturelles Muster (z.B. Injection, Broken Auth).
...
**Vektor**: Attacker-Position (Internet-Anon, Internet-User, Victim-Required, Build-Time, Repo-Read, n/a) aus `plugin/data/breach-vector-taxonomy.yaml`.
...
- `plugin/data/sections-contract.yaml` — the contract itself
- `plugin/schemas/fragments/README.md` — schema authoring guide
