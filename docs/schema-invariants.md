...
`data/sections-contract.yaml` is the human-edited declaration that every other map should align with; the maps duplicate fragments of it because each consumer reads only the slice it needs. Adding a new fragment means touching all five maps + the contract + the schema + the fragment's `.j2` template under `templates/fragments/` when it renders via one (the `_render_template` call in `docs/adding-a-section.md`). The mechanical sequence is documented in `docs/adding-a-section.md`. The automated drift gate lives in `scripts/check_fragment_registry.py` (see Phase A1 of the refactoring plan) — when present it MUST stay green in CI.
