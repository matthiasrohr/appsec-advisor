...
- Keep `agents/appsec-threat-analyst.md` thin. Detailed phase logic belongs in `agents/phases/`; compact runtimes, mode branches, and phase groups lazy-load only when reached.
...
- Route new event-log lines through `scripts/event_log.py` (`format_line`); do not hand-roll lines for `.agent-run.log`, `.hook-events.log`, or the no-session sentinel. Keep changelog bullets short, plain, user-visible, and consistent with the released `0.4.0-beta` tone.
...
- Merge LLM and deterministic threats through the same contract; prefer a deterministic emitter when it can own a category. Supply-chain analysis is passive: inspect files and git history, never run package-manager or network CVE scanners.
