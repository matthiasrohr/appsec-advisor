...
| `appsec-abuse-case-verifier` | Sonnet | Stage-1c fan-out. **`claude-sonnet-4-6` at `quick`/`thorough`; `claude-sonnet-5` at `standard` (buy-back)**; pin via `APPSEC_ABUSE_VERIFIER_MODEL`. 4.6 reintroduces `inconclusive` verdicts, so 5 stays the standard default. Opus banned. |
| `appsec-threat-renderer` | Sonnet | Fresh Stage-2 budget. **`claude-sonnet-4-6` at `quick`/`thorough`; `claude-sonnet-5` at `standard` (buy-back)**; pin via `APPSEC_RENDERER_MODEL`. Dispatched as two parallel calls (§7 ‖ MS) + single + recovery — each must set the Agent `model` param. |
| `appsec-qa-reviewer` | Sonnet | `qa_content` = `claude-sonnet-4-6` at `quick`/`thorough`, `sonnet` alias at `standard`; `qa_routine` Haiku at quick/standard, `claude-sonnet-4-6` at thorough. |
