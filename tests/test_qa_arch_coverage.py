...
def test_rule_visible_via_threat_hypotheses_passes():
    ...
    tm = {"threat_hypotheses": [{
        "id": "HYP-001",
        "rule_id": "ARCH-SQLI-001",
        "source_hypothesis_id": "ARCH-HYP-SQLI-001",
        "title": "x", "threat_category_id": "TH-01",
        "cwe": "CWE-89", "proof_state": "control-derived",
        "confidence": "medium",
    }]}
    assert qa.check_completeness(cov, tm, {}) == []
...
def _write_output_dir(tmp_path: Path,
                       coverage: dict | None,
                       threat_model: dict | None = None,
                       threats_merged: dict | None = None) -> Path:
    out = tmp_path / "out"
    out.mkdir()
...
def test_cli_passes_when_rule_visible(tmp_path: Path):
    cov = _coverage("anti_pattern", rule_id="ARCH-TLS-001")
    merged = {"version": 1, "generated_at": "x", "threats": [{
        "t_id": "T-001", "source": "architecture-coverage",
        "rule_id": "ARCH-TLS-001", "cwe": "CWE-319", "risk": "High",
    }]}
...
def test_cli_json_output_shape(tmp_path: Path):
    ...
    result = subprocess.run([sys.executable, str(SCRIPT), str(out), "--json"],
                            capture_output=True, text=True)
