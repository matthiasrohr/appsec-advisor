...
def test_representative_verdict_validates():
    ...
    verdict = {
        ...
        "summary": {
            "changed_files": 2, "candidates": 3, "in_scope": 2,
            "pass": 1, "partial": 0, "fail": 1, "unverifiable": 0, "not_applicable": 1,
            "gating_failures": 1,
        },
        "results": [
            {
                "id": "SEC-SQL", "category": "SEC-SECURE_DATA_HANDLING", "priority": "MUST",
                "status": "FAIL", "in_scope": True,
                "evidence": [{"file": "src/routes/search.ts", "line": 23}],
                "finding": "raw input reaches sequelize.query() at line 23",
                "fix": "bind the term via replacements", "effort": "M",
                "url": "https://asr.int.example.com/scg#sec-sql", "gating": True,
            },
            {
                "id": "SEC-CSP", "priority": "SHOULD", "status": "PASS", "in_scope": True,
            },
            {
                "id": "SEC-CORS", "priority": "MUST", "status": "NOT_APPLICABLE", "in_scope": False,
...
def test_schema_rejects_unknown_status():
    schema = json.loads(SCHEMA_PATH.read_text(encoding="utf-8"))
    bad = {
        "version": 1, "generated_at": "2026-06-07T00:00:00Z", "base_ref": "main",
        "priority_floor": "MUST",
        "summary": {"changed_files": 0, "candidates": 0, "in_scope": 0, "pass": 0,
                    "partial": 0, "fail": 0, "unverifiable": 0, "not_applicable": 0, "gating_failures": 0},
...
def test_build_verify_diff_on_staged_change(tmp_path):
    ...
    rc = build_verify_diff.main([
        "--repo-root", str(repo), "--output-dir", str(out_dir), "--staged",
    ])
    assert rc == 0
...
def test_fetch_falls_back_to_baseline_when_no_company_source(tmp_path):
    ...
    rc = fetch_requirements.main([
        "--caller", "verify-requirements",
        "--output-dir", str(tmp_path),
        "--plugin-root", str(ROOT),
        "--cache-path", str(tmp_path / "absent-cache.yaml"),
        "--require",
        "--fallback-baseline", str(BASELINE_PATH),
    ])
    assert rc == 0
...
def test_fetch_explicit_source_failure_does_not_fall_back(tmp_path):
    ...
    rc = fetch_requirements.main([
        ...
        "--output-dir", str(tmp_path),
        "--plugin-root", str(ROOT),
        "--requirements", str(tmp_path / "does-not-exist.yaml"),
        "--fallback-baseline", str(BASELINE_PATH),
    ])
    assert rc == 2
    assert not (tmp_path / ".requirements.yaml").exists() or \
        "bundled-bestpractices" not in (tmp_path / ".requirements.yaml").read_text(encoding="utf-8")
