...
def test_bridge_skips_unconfirmed_hypotheses() -> None:
    cov = _coverage_fixture(
        hypotheses=[
            ...
            {
                "hypothesis_id": "ARCH-HYP-XSS-001",
                ...
                "confidence": "high",
                "weak_or_missing_controls": ["Output Encoding"],
                "positive_signals": [],
                "decision": "emit_hypothesis_only",
            },
        ]
...
def test_end_to_end_bridge_via_cli(tmp_path: Path) -> None:
    ...
    (out / ".threats-merged.json").write_text('{"version":1,"generated_at":"2026-05-16T00:00:00Z","threats":[]}\n')
    subprocess.run(
        [
            sys.executable,
            str(BRIDGE),
            "merge-into",
            "--input",
            str(out / ".architecture-coverage.json"),
            "--threats-merged",
            str(out / ".threats-merged.json"),
        ],
        check=True,
        capture_output=True,
    )
...
def test_persist_cli(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        [sys.executable, str(BRIDGE), "persist-hypotheses", "--input", str(cov_path), "--threat-model", str(yaml_path)],
        capture_output=True,
        text=True,
        check=True,
    )
