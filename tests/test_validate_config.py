"""Unit tests for plugin/scripts/validate_config.py.
...
VALIDATE_CONFIG_PATH = (
    Path(__file__).parent.parent / "plugin" / "scripts" / "validate_config.py"
)


@pytest.fixture(scope="module")
def validate_config():
    """Import plugin/scripts/validate_config.py as a module."""
...
# ---------------------------------------------------------------------------
# Main plugin config (plugin/config.json)
# ---------------------------------------------------------------------------
...
class TestRealWorldConfigs:
    def test_actual_plugin_config_passes(self, validate_config):
        plugin_config = (
            Path(__file__).parent.parent / "plugin" / "config.json"
        )
        if not plugin_config.exists():
            pytest.skip("plugin/config.json not present in this checkout")
        import json
        with plugin_config.open() as fh:
            data = json.load(fh)
        errors = validate_config._validate_main_config(data, str(plugin_config))
        assert errors == [], f"Real plugin/config.json failed validation: {errors}"
