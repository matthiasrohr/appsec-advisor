...
`appsec-advisor` is a Claude Code plugin for AppSec teams, built around code-anchored **threat modeling**. It turns a repository into a security architecture model, identifies trust boundaries and data flows, applies STRIDE, and produces structured findings; on the same foundation, it also supports **requirements audits** and **developer helpers** (change review, pre-commit guidance, CI gates). Teams can use as-is or adapt with their own requirements, threat context, presets, guardrails, and company-branded packaging.
