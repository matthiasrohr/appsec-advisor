...
def apply_plan(md_path: Path, plan_path: Path, min_score: float, dry_run: bool) -> int:
    ...
    if dry_run:
        print(
            f"apply_finding_refs_repair: DRY-RUN — would apply {applied} swap(s), "
    ...
    print(
        f"apply_finding_refs_repair: applied {applied} remap(s) · "
...
def main() -> int:
    ...
    ap.add_argument("--dry-run", action="store_true", help="Show what would change without writing the file")
    args = ap.parse_args()
