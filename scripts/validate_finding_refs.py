...
SIGNATURE_TOKENS = {
    'sqlite', 'md5', 'xxe', 'ssrf', 'csrf', 'idor', 'jwt', 'localstorage',
    'eval', 'mongoose', 'marsdb', 'rate', 'limiting', 'metrics', 'prometheus',
    'review', 'basket', 'price', 'changepassword',
}

STOPWORDS = {
    'the', 'a', 'an', 'and', 'or', 'in', 'on', 'at', 'to', 'for',
    'of', 'with', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
    'via', 'this', 'that', 'these', 'those', 'from', 'as', 'by',
    'no', 'not', 'has', 'have', 'had', 'do', 'does', 'did',
}
...
def main() -> int:
    ...
    ap.add_argument('--strict', action='store_true',
                    help='Exit 1 when defects are detected (default: exit 0 with report only)')
    ap.add_argument('--remap', action='store_true',
                    help='Emit .finding-refs-repair-plan.json mapping bad refs to suggested fixes')
    ap.add_argument('--json', action='store_true',
                    help='Print JSON to stdout in addition to the report file')
    args = ap.parse_args()
    ...
    print(f'validate_finding_refs: scanned {sum(1 for p in candidate_paths if p.exists())} file(s) · '
          f'{len(all_defects)} defect(s) · report: {report_path}', file=sys.stderr)
    ...
    if args.strict and all_defects:
        for d in all_defects[:10]:
            print(f"  {d['fragment']}:{d['line']}  {d['defect']}  {d['f_id']}  "
                  f"→ remap to {d.get('remap_candidate')}", file=sys.stderr)
        return 1
    return 0
