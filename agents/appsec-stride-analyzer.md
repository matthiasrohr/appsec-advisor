...
**Component scope contract (Fix #7 — control-location tier classification + enforcement).** Each threat's `component_id` MUST equal the component whose `paths` globs include the threat's primary evidence file (`evidence[0].file`). The orchestrator now passes the **`COMPONENT_PATHS`** parameter — a comma-separated list of `paths` globs for THIS component. Before emitting any threat with a non-null `evidence[0].file`:

1. Match `evidence[0].file` against the `COMPONENT_PATHS` globs (use `fnmatch.fnmatch` semantics — `**` recurses, `*` is single-segment).
2. If the file matches at least one glob → emit the threat as usual.
3. If the file matches **none** of the globs → **do NOT emit the threat**. It belongs to a different component and will be analyzed (or already has been) in that component's STRIDE pass. The post-Stage-1 `reclassify_components.py` deterministic gate still catches drift, but every threat suppressed at source saves one round-trip downstream.

When `COMPONENT_PATHS` is missing (legacy orchestrator), fall back to the heuristic above (prefer the file's owning component over the abstract attack target). Common drift pattern observed: SQL injection found in `routes/search.ts` (an Express controller) recorded against `component_id=data-layer` because the vulnerability "ends up touching the DB" — that's attack-target-tier reasoning. The correct tier is control-location: the missing control (parameterised query, input validation) belongs to the file that issues the unsafe API call, i.e. `express-backend`.
...
| `started_at` (top-level, ISO 8601) | ~~omitting this field~~ — required for per-component duration (Fix #8). Set this at the START of Step 1 by capturing `$(date -u +%Y-%m-%dT%H:%M:%SZ)` and persisting it inline in your working notes; emit it as the second top-level field of the output JSON. Without it `record_component_durations.py` collapses every parallel STRIDE component to the same orchestrator-observed window and the next run's Phase-9 estimate cannot be per-component. |
...
```json
{
  ...
  "started_at": "<ISO 8601 timestamp captured at Step 1 start — REQUIRED for per-component duration tracking>",
  "analyzed_at": "<ISO 8601 timestamp captured at Step 4 just before write — REQUIRED>",
  ...
  "threats": [
    {
      ...
      "title": "<REQUIRED — canonical form: `<Weakness class> (<relative_file_path[:line]>)`. MAXIMUM 80 characters. This becomes the rendered label wherever the F-NNN ID is linked (Top Findings, §8 tables, §9 Addresses lists, §3 walkthrough headings). Rules: (1) The weakness class is the short noun phrase identifying WHAT the vulnerability is — 'SQL Injection', 'Hardcoded Cryptographic Key', 'Server-Side Template Injection', 'Insecure Direct Object Reference', 'Cross-Site Scripting'. Title-Case the leading word. (2) The location is the source-tree file path (with optional `:line`) in PARENS, never via em-dash. The path comes from `evidence[0].file`. (3) MUST NOT contain backtick code identifiers inside the title text — no inline `lib/...` ; the path goes inside the parens unquoted. (4) MUST NOT contain function-call expressions, payloads, library versions, exploit phrasing, or product-internal training-tier identifiers ('LEVEL_2', 'LEVEL_3 handler'). **Explicit forbidden substrings** (hard-fail by the schema's `bad_title_substrings` validator): `@0.`, `@1.`, `@2.`, `@3.` (any `lib@version` form), `alg:none`, `noent:true`, `bypassSecurityTrustHtml`, `crypto.createHash`, `eval(`, `models.sequelize.query`, `fetch(url)`, `app.use(`, `package-lock=false`, `(CVE-`. (5) When no file applies (cross-cutting / architectural-process), omit the parens entirely. Good examples: 'SQL Injection (routes/login.ts:34)', 'Hardcoded Cryptographic Key (lib/insecurity.ts:23)', 'Cross-Site Request Forgery (server.ts)', 'Outdated Dependency (package.json)', 'Insecure Token Storage (frontend/src/app/Services)', 'JWT Algorithm Confusion (lib/insecurity.ts:54)', 'XXE External Entity Parsing (routes/dataExport.ts:42)', 'Path Traversal via Archive Extraction (routes/fileUpload.ts:88)'. Bad examples: 'SQL injection — routes/login.ts:34' (em-dash separator before file — use parens), 'SQL Injection' (no file, no location — too generic), 'Reflected XSS via `bypassSecurityTrustHtml(queryParam)`' (function-call expression in title), 'JWT alg:none bypass — express-jwt 0.1.3 (CVE-2020-15084)' (library@version + payload phrase + em-dash + CVE), 'XXE via XML file upload — libxmljs2 noent:true' (library + payload phrase), 'MD5 password hash — offline cracking via SQLi dump' (exploit-narrative phrase, not weakness class), 'Path traversal via Zip Slip — unzipper@0.9.15' (library@version). When the affected parameter is meaningful, the optional sibling field `affected_parameter` stores the parameter name (`email`, `q`, `id`, `X-Forwarded-For`) — DO NOT cram it into the title string.>",
