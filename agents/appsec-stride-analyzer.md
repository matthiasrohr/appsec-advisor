  ...
  "threats": [
    {
      ...
      "title": "<REQUIRED — 2–6 word noun phrase, MAXIMUM 60 characters. This becomes the rendered label wherever the F-NNN ID is linked (Top Findings, §8 tables, §9 Addresses lists, §3 walkthrough headings). Rules: (1) MUST NOT contain backtick code identifiers — no `lib/insecurity.ts`, no `bypassSecurityTrustHtml()`. (2) MUST NOT contain file paths, route paths, line numbers, or function call expressions. (3) MUST identify WHAT the vulnerability is (type + location in plain words), not HOW it is implemented. (4) The location part (after 'in') names the feature/endpoint in plain English: 'in login', 'in search', 'in file upload', 'in B2B orders'. Good examples: 'Hardcoded RSA Private Key', 'SQL Injection in Login', 'Reflected XSS in Search', 'MD5 Password Hashing', 'JWT in localStorage'. Bad examples: 'SQL injection in `routes/login.ts:34` enables authentication bypass' (too long, has path+line), 'Reflected XSS via `bypassSecurityTrustHtml(queryParam)`' (has code snippet). **Do NOT embed product-internal training-tier identifiers** — never 'at LEVEL_2', 'LEVEL_3 handler', 'in LEVEL_9'. Name the handler/route instead in plain words (e.g. 'Plaintext Password Logging in Auth Handler').>",
