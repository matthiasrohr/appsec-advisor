...
**Overwrite the file after EVERY single step you resolve — never batch the write to the end.** The file is your *running state*, not a final artifact: the moment you upgrade one step's verdict from `inconclusive` to `confirmed`/`blocked` (or refine its `reason`/`evidence`), re-`Write` the whole file immediately, then continue to the next step. A verifier that investigates all steps and only writes at the end loses ALL its work if it hits the turn ceiling one step short — exactly what happened to the AC-T-002 IDOR case on 2026-06-12 (it traced four steps of middleware ordering, hit `maxTurns`, and left the untouched pre-seed: both steps `inconclusive`, empty excerpts). Per-step writes make every cut-off degrade to "as far as I got", not "nothing".
