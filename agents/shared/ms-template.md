...
| Centralised Session Validation | `express-jwt` middleware mounted at the API root | ⚠️ Partial | Algorithm allow-list permits `none` | [T-002](#t-002) — alg:none bypass |
| <... 3–8 rows total, one per existing control with effectiveness ≥ Weak; OMIT any HTTP-header / response-header / CSP / CORS row even if Section 7 lists one ...> | | | | |
