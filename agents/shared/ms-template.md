...
- **Verified-chain badge (renderer-added, do NOT author).** When a bullet's `refs` anchor a fully-viable abuse chain (`.fragments/abuse-cases.json` → `chain_verdict == "fully_viable"`), the renderer appends ` — ✓ **end-to-end verified** ([AC-NNN](#ac-nnn))` to that bullet, linking §9. This marks the code-proven subset without a separate line; it never contradicts the red box (badge appears only for genuinely viable chains) and is silently absent at quick depth / `--no-abuse-cases`. The LLM must not write this badge — it is computed deterministically in `compose_threat_model._verdict_bullet_badge`.
...
<Closing line linking to §6: `See **[§6 Security Architecture](#6-security-architecture)** for the full per-domain assessment …`>
...
<⚠ MANDATORY 3-column **cluster** table. The columns are `Strength`, `What's in Place`, `Effectiveness`. Each row is a **categorical cluster** (e.g. `Container & Supply-Chain Hardening`, `Authentication & Session Management`, `Hardened HTTP Stack`, `Other Operational Controls`) drawn from `$CLAUDE_PLUGIN_ROOT/data/strength-clusters.yaml`; the `What's in Place` cell aggregates 1–7 underlying controls separated by `<br/>`, each carrying evidence in the form `Control Name - file:line - mechanism; gap` (the legacy 5-column form with `Architectural Control / Implementation / Effectiveness / Gap / Mitigates` is **retired** as of 2026-05 — it bloated the table and pushed evidence out of view). Underlying control names MUST be canonical from `$CLAUDE_PLUGIN_ROOT/data/architectural-controls.yaml`. Effectiveness of the cluster row equals the BEST effectiveness among its members; rows with only `❌ Missing` members do not appear here — those live in Section 7 only.>
...
- Deployment-time / runtime-environment controls that a source-tree scan cannot verify (WAF, IDS/IPS, API gateway, network firewall, secret scanning service, EDR/SIEM) — see the "Closing-sentence rule" above; the same prohibition applies here.

If you're tempted to add an HTTP-header row because the table looks thin, prefer instead to keep the table at 3–4 strong rows and lean on the `**Bottom line:**` sentence to acknowledge the modest defensive surface. Padding with hygiene rows weakens the section's signal.

| Strength | What's in Place | Effectiveness |
|---|---|---|
| **Container & Supply-Chain Hardening** | Distroless Runtime Image - Dockerfile - runtime stage on distroless; reduces attack surface<br/>Angular Template Escaping - Angular default escaping active; explicitly bypassed via `bypassSecurityTrustHtml` in five components | ✅ Adequate |
| **Authentication & Session Management** | Route Authorization Middleware - server.ts - `security.isAuthorized()` on most routes; commented out on `PUT /api/Products/:id`<br/>Sequelize ORM - most queries via Sequelize; raw string interpolation on login + search<br/>Multi-Factor Authentication - 2fa routes - TOTP via `otplib`; not enforced on OAuth path<br/>Session Token Expiry - lib/insecurity.ts - 6-hour expiry on issued tokens; no revocation mechanism | ⚠️ Partial |
| **Hardened HTTP Stack** | Helmet Security Headers - server.ts - provides noSniff, frameguard, HSTS; xssFilter commented out and CSP missing<br/>Authentication Rate Limiting - `express-rate-limit` on reset + 2FA; not on `/login` and key derived from spoofable `X-Forwarded-For` | ⚠️ Partial |
| **Other Operational Controls** | Dependabot Configuration - `.dependabot/config.yml` present but explicitly ignores 4 known-vulnerable dependencies | 🔶 Weak |
| <... 3–6 cluster rows total; OMIT any cluster whose only members are HTTP-header hygiene ... > | | |
...
|---|---|---|
| `Strength` | **Cluster name** in bold (e.g. `**Container & Supply-Chain Hardening**`, `**Authentication & Session Management**`, `**Hardened HTTP Stack**`, `**Other Operational Controls**`) | `strength-clusters.yaml → clusters[].name` |
| `What's in Place` | 1–7 canonical control entries from `architectural-controls.yaml`, separated by `<br/>`. Each entry follows `<Canonical Control Name> - <file:line> - <mechanism>; <gap>` — the mechanism + gap clauses come from Section 7 evidence, NOT a library@version string. | Aggregated from Section 7 rows whose `domain` matches the cluster |
| `Effectiveness` | One of ✅ Adequate · ⚠️ Partial · 🔶 Weak (never Missing). **Cluster effectiveness = BEST effectiveness among its members** (Adequate beats Partial beats Weak). A cluster with at least one Weak member rendering as Adequate would mis-sell the posture — use the worst when ALL members are below adequate, the best otherwise. | Computed by `compose_threat_model.py:_build_strength_clusters` |

**Relationship to Section 7 (Identified Security Controls).** Operational Strengths is a **cluster-aggregated view** of Section 7 — each `What's in Place` line resolves to one Section 7 control with `effectiveness ∈ {adequate, partial, weak}`. `❌ Missing` controls live only in Section 7 and never appear in an Operational Strengths cluster row. The QA reviewer Check 3i validates that every underlying control name in Operational Strengths appears verbatim in Section 7 (same canonical name), that the cluster name comes from `strength-clusters.yaml`, that every `What's in Place` line carries evidence (`<Name> - <file:line> - <mechanism>; <gap>`), and that no Missing-only cluster appears here.
...
- **Required LLM-authored sub-sections — exactly FIVE, presence and order enforced by the template:** `### Verdict`, `### Top Findings`, `### Architecture Assessment`, `### Mitigations` (with `#### Prioritized Mitigations` and `#### Follow-up Mitigations` sub-tables), `### Operational Strengths`. The `### Requirements Compliance` sub-section is mandatory **only** when `CHECK_REQUIREMENTS=true` and is placed between Mitigations and Operational Strengths. **Do NOT author `### Top Weaknesses`** — the composer injects it deterministically from `weaknesses[]` immediately before the Mitigations tables (2026-07-14 Weakness-Register redesign). It is a computed executive view of the root-cause control gaps; authoring your own would be overwritten/duplicated.
...
- **Operational Strengths MUST be a 3-column cluster table** with columns: `Strength`, `What's in Place`, `Effectiveness`. The legacy 5-column form (`Architectural Control / Implementation / Effectiveness / Gap / Mitigates`) is **retired** as of 2026-05 — QA Check 3i auto-rewrites detected 5-column tables back to the 3-column cluster form. A 2-column or 4-column table is FORBIDDEN. The table targets 3–6 cluster rows; each cluster row aggregates 1–7 underlying canonical controls in the `What's in Place` cell, `<br/>`-separated, each line carrying `<Canonical Control Name> - <file:line> - <mechanism>; <gap>` evidence. **Forbidden as cluster rows** — any cluster whose only members are HTTP response-header hardening (`helmet`, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `HSTS`), and any control whose `architectural-controls.yaml` entry carries `excluded_from_strengths: true` (the renderer filters these out automatically; the QA reviewer flags them when they slip through). Cluster effectiveness = BEST effectiveness among the cluster's members. Padding with hygiene rows to reach a row count is worse than a shorter, stronger table. Every underlying control name MUST match a canonical name from `architectural-controls.yaml`; cluster names MUST match a cluster from `strength-clusters.yaml`. Ends with a `**Bottom line:**` sentence. The introductory paragraph before the table is mandatory when verdict is 🟡 or 🔴.
