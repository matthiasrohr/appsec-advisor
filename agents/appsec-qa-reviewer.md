...
**Legacy fallback path — only when the deterministic pre-pass failed (BASH_WARN logged).** Re-run `qa_checks.py links "$OUTPUT_DIR/threat-model.md" "$REPO_ROOT"` and parse its JSON output (same `links.issues` shape as above). The Python helper handles extraction, existence checks, basename-based auto-repair, and `find`-based candidate resolution. Do not re-implement this in Bash.
...
1. **Single-pass extraction (1 turn):** Read `$OUTPUT_DIR/threat-model.md` and extract ALL of the following into memory:
   ...
   - All T-NNN/F-NNN IDs from `## Critical Attack Tree` Quick-reference table rows
...
**3c — Critical Findings coverage (Attack Tree + Section 8.B)**

> **Layout:** Critical Findings content lives in three places — `## Critical Attack Tree` (unnumbered, after Management Summary, shows how Critical findings decompose into the attacker's terminal goal via AND/OR refinement), Section 3 (Attack Walkthroughs — sequenceDiagram per Critical finding), and Section 8.B Critical Categories (authoritative per-TH-NN tables in the Threat Register).

Locate the Quick-reference table inside `## Critical Attack Tree` (grep for the heading, then find the first `| ID |` table header line between that heading and the next `## ` heading). Call this `ATTACK_TREE_TABLE`. If `## Critical Attack Tree` is absent (0 or 1 Critical findings present → section is intentionally omitted), skip this check and proceed to 3d.

1. Extract all T-NNN referenced in `ATTACK_TREE_TABLE` rows. Count them as `TREE_COUNT`.
2. **Reverse check — Critical threats (auto-fix):** for each T-NNN in the Threat Register with Risk = **Critical** that is not already in `ATTACK_TREE_TABLE`, **add one row to the Quick-reference table** using this template (fill values from the matching Section 8 finding row — never duplicate prose):
   ```markdown
   ...
   ```
   Append the row at the **end** of the Quick-reference table — do not reorder existing rows. Print: `[qa-reviewer]   ↳ Added missing Critical threat to Critical Attack Tree: T-xxx`
3. **Forward check** — any T-NNN in `ATTACK_TREE_TABLE` that does not exist in the Threat Register: add `<sup>⚠ T-xxx not found in Threat Register</sup>` in the row. Print: `[qa-reviewer]   ↳ Orphaned T-ref in Critical Attack Tree: T-xxx`
4. For each T-NNN row in `ATTACK_TREE_TABLE`, verify the Mitigation cell contains an `M-NNN` link. If absent: add `<!-- QA: Critical finding T-xxx has no Mitigation link in the Attack Tree table — link to [M-NNN](#m-NNN) -->`.
5. **Back-link from Mitigation to Critical Finding:** For each T-NNN in `ATTACK_TREE_TABLE`, find its corresponding M-NNN entries in Section 9. If the mitigation addresses a Critical-rated threat and does not contain the threat ID in the `**Addresses:**` line, add `<!-- QA: M-xxx addresses Critical threat T-xxx — ensure Addresses line links back -->`.
...
Apply the rules from `shared/qa-crossref-rules.md` — they cover (1) the title lookup built from Threat Register rows, Mitigation headings, Finding/Component/Category anchors, (2) the reference-site classification table, (3) the four auto-repair rules in fixed order (bare-ID → link, link-without-em-dash → titled, comma-list outside tables → bullets, comma-list inside tables → `<br/>`), and (4) the edge-case exclusions (code fences, HTML comments, Mermaid blocks, anchor definitions, ID columns).

Run the check exactly once per QA pass. Apply repairs in a single `Edit` batch per file.

**Print**: `[qa-reviewer]   ↳ Style 3f: <n_title_fixes> titles added, <n_bullet_fixes> bullet-list conversions, <n_br_fixes> comma→<br/> conversions`.

**Log CHECK_END**: `Check 3 — 3f style fixes applied: <n> titles, <n> bullet lists, <n> table breaks`.
...
**Legacy strip rule.** The heading rename `### Top Threats → ### Top Findings` is handled deterministically by the pre-pass (`qa_checks.py` legacy-heading rename map). If a category-level table (header `| Severity | Category |` or `| # | Severity | Category |`) survives the rename — i.e. the rename happened but the old category-aggregated body remains — remove the entire surviving table and emit `<!-- QA: legacy Top Threats category table removed — unified Top Findings is now the sole MS register -->`. Category-level information remains accessible in §8.A.
...
**Print when done:** `[qa-reviewer]   ↳ Cross-references: <n> T/F→M links verified, <n> M→T/F back-links verified, <n> broken, <n> asymmetric, <n> critical added to Attack Tree, <n> req refs validated, <n> unknown req refs, <n> Section 9 missing req line, <n> 3f style fixes, <n> 3g classification tags, <n> 3h Top Findings rows reordered, <n> 3i Operational-Strengths repairs, <n> 3j category coverage repairs, <n> 3k autolink guards, <n> 3l §7.X heading violations`
...
5. **Critical findings count** — count data rows in the `## Critical Attack Tree` Quick-reference table (rows starting with `| [T-` or `| [F-` under the table header). Compare to `critical_findings:` list length in YAML and also to the number of Critical-rated findings in Section 8. All three counts must match. If they differ, add `<!-- QA: critical_findings count mismatch — YAML has <n>, Section 8 Critical findings has <n>, Critical Attack Tree has <n> -->` at the top of `## Critical Attack Tree`. When `## Critical Attack Tree` is absent (Critical count < 2), compare only YAML and Section 8.
...
| `## 7. Security Architecture` | Present and contains `### 7.1 Security Control Overview` followed by the v2 control-category sub-sections through `### 7.13 Defense-in-Depth Summary`. The legacy heading `## 6. Trust Boundaries` is **auto-stripped**. The legacy heading `## 7. Identified Security Controls` is **auto-renamed** to `## 7. Security Architecture`. |
| `## 8. Threat Register` | Present and contains a Markdown table with ≥ 1 data row |
| `## Critical Attack Tree` | Present when Threat Register has ≥ 2 Critical rows. Must contain a `\`\`\`mermaid` `graph TD` block (goal-decomposition tree) and a `| ID \| Title \| …` quick-reference table. Omitted entirely when Critical count < 2. The legacy `## Critical Attack Chain` heading is **auto-renamed** to `## Critical Attack Tree`; the legacy `#critical-attack-chain` anchor is preserved via a dual HTML anchor above the heading. |
...
**Section 7 structure check:** Section 7 (Security Architecture) should contain `### 7.1 Security Control Overview` as its first sub-section and the full v2 13-section control-category sequence through `### 7.13 Defense-in-Depth Summary`. If the legacy heading `## 7. Identified Security Controls` is found, **auto-rename** it to `## 7. Security Architecture`. If `### 7.1 Security Control Overview` is absent, add `<!-- QA: Section 7 is missing the v2 7.1 Security Control Overview sub-section — re-render from data/sections-contract.yaml → schema_v2.required_subsections -->`. **Do NOT flag a missing `**Gap summary:**` paragraph** — the Gap-Summary block is not part of v2; if one is still present from a stale fragment, flag it for removal instead: `<!-- QA: Section 7 carries a deprecated **Gap summary:** block — remove it; the v2 overview table and §7.13 summary replace it -->`. Print: `[qa-reviewer]   ↳ Section 7 structure: heading=<ok|renamed>, 7.1-overview=<ok|missing>, deprecated-gap-summary=<absent|present>`
...
**Section 7 Gap-Summary deprecation check:** The `**Gap summary:**` paragraph is not part of v2. If a paragraph beginning with the literal label `**Gap summary:**` is found inside §7 (anywhere between `## 7. Security Architecture` and `### 7.1 Security Control Overview`, or at the top of `### 7.1 Security Control Overview`), flag it for removal: `<!-- QA: Deprecated **Gap summary:** paragraph found — remove it. The v2 overview table and §7.13 Defense-in-Depth Summary replace it. -->`. Print: `[qa-reviewer]   ↳ Section 7 deprecated Gap-Summary: <absent|present>`

**Critical Attack Tree — attack-tree diagram required:** Count Critical-rated rows in the Threat Register (Section 8). If `>= 2`, the document must contain a `## Critical Attack Tree` heading (unnumbered, positioned between the Management Summary and Section 1) and that block must contain a ```` ```mermaid ```` `graph TD` block (goal-decomposition tree) before the `| ID |` Quick-reference table. The block MUST be preceded by the dual anchor lines `<a id="critical-attack-chain"></a>` and `<a id="critical-attack-tree"></a>` for backwards compatibility with the legacy deep-link. If the legacy heading `## Critical Attack Chain` is found, **auto-rename** it to `## Critical Attack Tree` and add the dual anchor lines above it. If the heading is missing entirely, add `<!-- QA: <n> Critical findings exist but the "## Critical Attack Tree" block is missing — it must be placed directly after the Management Summary, containing a Mermaid graph TD tree (root = attacker terminal goal, leaves = Critical findings, AND/OR refinement on edges) and the Quick-reference table. See phase-group-threats.md → "Critical Attack Tree layout" -->` directly after the Management Summary closing (before the first `## 1.` heading). If the heading is present but no Mermaid block is inside it, add the same comment under the heading. If a `graph LR` Mermaid is found under this heading (legacy chain layout), flag it: `<!-- QA: Critical Attack Tree must use graph TD (top-down goal-decomposition) — graph LR is the chain layout and is reserved for §3.1 Attack Chain Overview. Convert to tree form. -->`. If Critical count < 2, the section must be **absent** — its absence is correct, not a warning. Print: `[qa-reviewer]   ↳ Critical Attack Tree: <present+graphTD|present-graphLR|present-no-diagram|missing|legacy-heading-renamed|not required (<n> critical)>`

**Critical Attack Tree — no per-finding prose blocks:** The `## Critical Attack Tree` block is deliberately thin — only the intro sentence, the Mermaid diagram, the Key takeaway sentence, an optional Mitigation breakpoints bullet list, and the Quick-reference table. Per-finding prose blocks (Scenario / Current state / Violated Requirements) must live in the Threat Register (Section 8), not here. If the block contains any `### T-\d+`, `### F-\d+`, `### 🔴 T-\d+`, or `### 🔴 F-\d+` headings (the old per-finding prose format), flag each occurrence with `<!-- QA: Critical Attack Tree must not contain per-finding prose blocks — those live in Section 8 Threat Register. Replace with a row in the Quick-reference table. See phase-group-threats.md → "Rules for ## Critical Attack Tree" -->`. Print: `[qa-reviewer]   ↳ Critical Attack Tree duplication: <n> per-finding prose blocks flagged`

**Section 8 legacy-stub check:** Section 8 is the Threat Register in the current contract. If it still renders as the old two-line stub that only points to `Critical Attack Tree` and `Section 7.1 Critical`, add `<!-- QA: Section 8 is still using the legacy stub layout — re-render with the current Threat Register contract (8.A categories at a glance plus 8.B-8.E finding blocks). -->` at the top of Section 8. Print: `[qa-reviewer]   ↳ Section 8 legacy stub: <absent|present>`

**Section 2.4 — Security Architecture Assessment checks:** Apply the rules from `shared/qa-section24-themes.md`. Three sub-checks run in order:

1. **Numbered layout (1):** validate that Section 2.4 (also accepted as `### 2.3` / `### 2.5`) contains the nine canonical `#### 2.4.1`–`#### 2.4.9` H4 sub-sections; rewrite legacy non-numbered or H5-form headings; flag missing themes; verify reading order.
2. **Theme body format (2.4.3–2.4.8):** validate the bullet-first micro-template (`Current state.` / `Structural defects:` / `Impact.` / `Target architecture.` / `Linked threats:`), prose length, mandatory code refs in themes 2.4.3/4/5, library-name rules, theme length cap, and the sound-architecture short form.
3. **Per-theme diagrams (2.4.3–2.4.8):** wrong diagram type, prohibited-theme diagrams in 2.4.6 / 2.4.8, node-count cap (≤7), node-label rules, missing `**Key takeaway:**` line, and the mandatory-diagram matrix keyed by `DIAGRAM_DEPTH`.

Each sub-check prints its own `[qa-reviewer]   ↳ Section 2.4 ...` summary line as documented in `shared/qa-section24-themes.md`.

**Management Summary checks (10 sub-checks):** Apply the rules from `shared/qa-ms-checks.md`. Run the sub-checks in order:

1. **Presence** — generate the whole MS from the Threat / Mitigation Registers if `## Management Summary` is missing entirely (critical defect).
2. **Verdict** — `### Verdict` first; severity cue + red HTML blockquote with bullets + closing sentences; auto-wrap legacy plain bullets.
3. **Required sub-sections** — exactly five in order (Verdict, Top Findings, Architecture Assessment, Mitigations, Operational Strengths); plus `### Requirements Compliance` when `CHECK_REQUIREMENTS=true`; strip numeric prefixes.
4. **Forbidden sub-sections** — auto-strip Risk Distribution / STRIDE Coverage / Worst Case Scenarios; auto-rename Top Threats / Critical Findings / Key Strengths; flag the rest.
5. **Verdict blockquote** — red HTML blockquote with 2–5 bold-name bullets + parenthesised italic F-NNN citations; migrate any standalone Worst Case Scenarios heading into it.
6. **Top Findings table** — 7 columns (`# | Criticality | Finding | Component | Threat | Vektor | Primary Mitigations`); 🔴 before 🟠; legend line required.
7. **Architecture Assessment table** — 3 columns (`Defect | Description | Key Findings`); legacy 5-column form is logged but not auto-rewritten.
8. **Mitigations sub-tables** — `#### Prioritized Mitigations` + `#### Follow-up Mitigations`, both 5-column (`ID | Mitigation | Component | Addresses | Effort`); legacy `### Follow-up Actions` auto-rewritten.
9. **Operational Strengths format** — 3-column cluster header only (detailed cluster validation is Check 3i).
10. **Prose purity** — no `[T-` / `[M-` / `vscode://` / file paths in Verdict prose or Architecture Assessment intro.

Each sub-check prints its own `[qa-reviewer]   ↳ Management Summary …` summary as documented in `shared/qa-ms-checks.md`.
...
**Label-as-code (R-7, 2026-05) — INVERSE check:** Unwrap backticks from tokens that are LABELS / FIELD NAMES / bare HTTP-method nouns, NOT code fragments. Specifically the following single-word tokens MUST appear bare (no backticks) in prose:

- **MS / register field labels:** `Why`, `How`, `Effort`, `Priority`, `Severity`, `Addresses`, `Component`, `Components`, `Mitigation`, `Mitigations`, `Notes`, `Vektor`, `Classification`, `Issue`, `Impact`, `Fix`, `Location`, `Evidence`, `Verification`, `Steps`
- **Lowercase schema field references:** `notes`, `addresses`, `priority`, `effort`, `severity`
- **Bare HTTP method nouns** when written as a noun, NOT as part of a request: `GET`, `POST`, `PUT`, `DELETE`, `PATCH`, `HEAD`, `OPTIONS`. (Inside an actual request example like `POST /api/Users` the path stays in backticks but the method itself does not.)
- **Bare `verify`** — if you mean the JWT verification call, write it as `jwt.verify()` so the parens make it unambiguously code.

If a token in this list appears between `` ` `` in prose, strip the backticks. The deterministic `apply_prose_fixes.py` pass and `qa_checks.py label_as_code` already detect and auto-fix these, but the QA reviewer should still scan and confirm. Print the count in the inline-code summary line.

Print: `[qa-reviewer]   ↳ Code formatting: <n> tokens wrapped, <n> title contexts skipped, <n> label-as-code unwrapped`
...
- `## Critical Attack Tree` Quick-reference table rows (T-NNN in the ID column)
