...
- **Requirements Compliance count consistency.** When `CHECK_REQUIREMENTS=true`, find the `**Result:**` line in the Management Summary `### Requirements Compliance` sub-section and the §7b `**Summary:**` line. All reported status counts must match, including PASS, FAIL, PARTIAL, ANTI-PATTERN, N/A, NOT OBSERVABLE, and UNVERIFIABLE when present. Mismatch → `<!-- QA: Requirements Compliance counts differ — MS says <tuple>, §7b says <tuple> -->`.
- **Fulfills Requirements completeness.** For each mitigation, the union of requirement IDs across its addressed threats that are FAIL/PARTIAL/ANTI-PATTERN in §7b must appear on the mitigation's `**Fulfills Requirements:**` line. PASS, N/A, NOT OBSERVABLE, and UNVERIFIABLE requirements must not appear there.
...
| `**Blueprint guidance:**` containing `[BP-` | when `.requirements.yaml` has top-level `blueprints:` AND ≥1 addressed violated requirement maps to blueprint guidance | line starts with `**Blueprint guidance:**` and contains `[BP-` |
| `**Fulfills Requirements:**` | when `CHECK_REQUIREMENTS=true` AND ≥1 addressed threat carries a FAIL/PARTIAL/ANTI-PATTERN `Violated:` reference | line starts with `**Fulfills Requirements:**` |
