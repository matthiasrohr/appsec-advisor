...
**If MODE=update and CHANGED_FILES ≠ ALL:** Apply update mode rules (see Update Mode Rules table — Phase 2 trigger patterns). For layers whose files did not change, copy the corresponding diagram from the prior threat model verbatim with an inline annotation: `<!-- carried forward from PRIOR_GENERATED -->`. Re-generate only layers with changed files.
