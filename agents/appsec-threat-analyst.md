...
**## 6. Use Cases** — REMOVED 2026-05. The numbering gap (§5 → §7) is intentional. Do NOT author a §6 section, do NOT populate `use_cases[]` in the YAML — the field is no longer rendered by the composer. Restoration would require coordinated edits in `data/sections-contract.yaml`, `scripts/pregenerate_fragments.py`, and `scripts/compose_threat_model.py`.
...
- **No redundancy between Critical Attack Tree and Section 3.** The Critical Attack Tree (before Section 1) shows how Critical findings **decompose into one attacker goal** — a single `graph TD` diagram with AND/OR refinement. Section 3 (Attack Walkthroughs) shows **each finding in detail** — one `sequenceDiagram` per Critical finding. Do not duplicate diagrams or tables between these two sections.
- **§3.1 Attack Chain palette (hard contract — `chain_compactness` in `data/sections-contract.yaml`).** §3.1 chains use a DIFFERENT `classDef risk`/`impact` than the §2 architecture diagrams above. Each `graph LR` chain block MUST contain these two `classDef` lines verbatim:
  ```mermaid
  ...
  ```
  Apply `:::risk` to every finding/attacker-step node and `:::impact` to the terminal business-impact node. The §2 architecture `classDef risk fill:#FFB6C1` is for §2 only — do NOT reuse it in §3.1. (Both color schemes use the name `risk` but the contract distinguishes them; mermaid scopes classDef to one fenced block so there is no collision at render time as long as each block declares its own classDef inside the fence.)
- **§3.1 chain node label keyword consistency.** Every chain node that cites `T-NNN` MUST share at least one distinctive content keyword with that threat's `title` field in `threat-model.yaml`. Embed the actual sink name (e.g. `bypassSecurityTrustHtml`, `jws.verify`, `runInContext`), the actual library (`libxmljs2`, `marsdb`), or the file basename (`search-result.component`, `b2bOrder.ts`) so the cross-reference is verifiable. Generic verbs (`renders`, `extracts`, `processes`) without an evidence keyword are a `chain_tid_consistency` defect.
