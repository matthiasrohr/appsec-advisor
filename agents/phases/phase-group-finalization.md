...
**Audience split — the Verdict is for the PRODUCT OWNER, the rest is for engineers.** The `### Verdict` block (`ms-verdict.json` → `opening` / `bullets[].body` / `closing`) is the ONE part of the report a non-technical project lead / product owner reads. Keep it at **management altitude**: characterise the application's overall posture and what an attacker could achieve in business terms (access other customers' accounts, run arbitrary code, exfiltrate data, take over admin), and where controls are missing describe a *class of gap* — "the application skips several standard security practices, e.g. server-side authorization and input validation" — NOT a config-level finding. **Do NOT put into the Verdict:** specific config keys (`httpOnly`, `sameSite`, `sslmode`), library names or versions (`lodash@4.17.10`), CVE/CWE numbers, file names or `file:line` refs, framework-internal symbols (`_.merge`), or port numbers. Those belong in §8 Findings and §7 — the Verdict's `bullets[].refs` already carry the F-/T-NNN pointer for any reader who wants the detail.
