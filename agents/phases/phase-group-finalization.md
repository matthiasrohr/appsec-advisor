...
| Attack Walkthroughs (§3) | `.fragments/attack-walkthroughs.md` | Plain Markdown with at least one `sequenceDiagram` per Critical finding. **Chain anchors must be on a separate line above the heading** (`<a id="chain-1"></a>\n#### Chain 1 — …`), NOT inline with the `####` text — inline `<a>` tags break right-side TOC outline panels in many markdown viewers. Same convention for any other `####` heading that needs a stable explicit anchor. | Required-pattern validation. |
