...
| Critical Attack Tree (REQUIRED — 2026-05-28 activation) | `.fragments/ms-critical-attack-tree.json` | `{intro, root_goal?, mermaid{orientation:TD|TB,nodes[],edges[]}, key_takeaway, mitigation_breakpoints?[], stages[]}` — schema-validated against `schemas/fragments/critical-attack-tree.schema.json`. **MANDATORY when `critical_count >= 2`** (renderer wired in `compose_threat_model.py` dispatcher since 2026-05-28). Skip silently when fewer than 2 Critical findings exist. Build the tree from the `threats[].risk == Critical` rows in `threat-model.yaml`: each Critical finding becomes a leaf node, AND-grouped nodes describe chained preconditions (e.g. `T-001 hardcoded key` + `T-003 alg:none` → `Forge admin JWT`), OR-grouped nodes show alternative leaves leading to the same goal. The root goal node names the worst-case impact (`Full admin takeover`, `Full DB exfiltration`, …) and matches one of the Worst-Case-Scenario bullets in the Verdict fragment. | Mermaid `graph TD` (goal-decomposition with AND/OR refinement), Stage/Finding/Mitigation table, unnumbered `## Critical Attack Tree` with dual anchors `#critical-attack-tree` (canonical) and `#critical-attack-chain` (legacy, preserved for backwards compatibility). |
...
**Unnumbered** `## Critical Attack Tree` section, placed **immediately** after the Management Summary and **before** Section 1. Contains: one attack-tree Mermaid diagram (`graph TD`, goal-decomposition with AND/OR refinement) + "Key takeaway" sentence + optional "Mitigation breakpoints" bullet list + quick-reference table linking back to Section 7.1. The canonical anchor is `#critical-attack-tree`; the legacy anchor `#critical-attack-chain` is preserved via an explicit HTML anchor immediately above the heading (AGENTS.md §5 — ID stability). Omit the section entirely when there are 0 or 1 Critical findings (a single Critical cannot form a meaningful tree).
