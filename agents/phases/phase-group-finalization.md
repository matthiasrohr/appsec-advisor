...
| Critical Attack Tree | `.fragments/ms-critical-attack-tree.json` | `{intro, root_goal?, mermaid{orientation:TD|TB,nodes[],edges[]}, key_takeaway, mitigation_breakpoints?[], stages[]}` — schema-validated against `schemas/fragments/critical-attack-tree.schema.json` | Mermaid `graph TD` (goal-decomposition with AND/OR refinement), Stage/Finding/Mitigation table, unnumbered `## Critical Attack Tree` with dual anchors `#critical-attack-tree` (canonical) and `#critical-attack-chain` (legacy, preserved for backwards compatibility). |
