...
> ⛔ **Never re-dispatch the context-resolver or recon-scanner a second time.** Once these foreground agents have returned, their work is done — *even if* a subsequent Bash existence check shows `.threat-modeling-context.md` or `.recon-summary.md` as **missing**. That false-missing happens when a long model/API stall recovers (the orchestrator rechecks after the standby) or when it rechecks before the agent's final `Write` has flushed. A missing output is resolved by the **fallback branches below — never by spawning the same agent again**: recon-scanner missing → minimal inline scan (see "If `.recon-summary.md` is missing", below); context-resolver missing → halt if `CHECK_REQUIREMENTS=true`, else proceed without it. A second `appsec-recon-scanner` / `appsec-context-resolver` Agent call (the "Re-run … to write output file" pattern) re-pays the full context prefill for zero new analysis — it is the single most common token-waste regression on stalled runs and is a contract violation, not a recovery step.
...
If `.recon-summary.md` is missing **after the recon-scanner agent has already returned**, fall back to minimal inline scan. Do **not** check for `.recon-summary.md` while the recon-scanner is still running and use its absence as a reason to re-dispatch — the file will appear once the agent finishes writing it. This also holds **after a model/API stall recovers**: a file that reads as missing post-stall is still the inline-scan fallback, **never** a second `appsec-recon-scanner` dispatch (see the ⛔ rule under "Execution protocol" step 4).
