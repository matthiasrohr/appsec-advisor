...
- `.fragments/ms-ai-exposure.json` when `KNOWN_LLM_PATTERNS` is non-empty and you found ≥1 LLM-specific threat in STRIDE. Map each threat to its OWASP LLM Top-10 category (see `### ms-ai-exposure.json authoring contract` below). **Omit the file entirely** when no LLM/AI surface exists or no LLM threats were found. This is a REQUIRED output for any component that had `KNOWN_LLM_PATTERNS` set — missing it leaves the `### AI / LLM Exposure` Management Summary section blank.
