...
The legal title shape is the storage form `<Weakness class> (<relative_file_path[:line]>)` for one instance, or class-only for consolidated multi-location findings. The rendered `Instances (N)` row owns their paths. The library + version + payload phrasing belongs in the finding's `evidence.excerpt` or in the §7 narrative prose — never in the title.
