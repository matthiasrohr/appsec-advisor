...
```bash
APPSEC_VERBOSE=1 claude --plugin-dir /path/to/appsec-plugin
```
...
- `source: stride` → CVSS vector **allowed** iff (a) the threat's CWE appears in `data/cvss-eligible-cwes.yaml` (injection, XSS, SSRF, path traversal, deserialization, auth bypass, hardcoded credentials, crypto misuse, and similar concrete-sink weaknesses) **and** (b) `evidence.file` + `evidence.line` point at an exploitable code location. The STRIDE analyzer emits vectors only when both conditions hold.
