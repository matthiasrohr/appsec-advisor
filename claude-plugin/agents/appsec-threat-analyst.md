...
8. **Resolve `CLAUDE_PLUGIN_ROOT`** — try common install paths first (O(1) each), fall back to `find` only if needed. **Combine this Bash call with the stale-file cleanup above in the same turn:**
   ...
   if [ -z "$CLAUDE_PLUGIN_ROOT" ]; then
     for d in "$HOME/github/appsec-plugin/claude-plugin" "$HOME/.claude/plugins/appsec-plugin/claude-plugin" "/opt/appsec-plugin/claude-plugin" "/appsec-plugin/claude-plugin"; do
       [ -f "$d/config.json" ] && CLAUDE_PLUGIN_ROOT="$d" && break
     done
   fi
   if [ -z "$CLAUDE_PLUGIN_ROOT" ]; then
     CLAUDE_PLUGIN_ROOT=$(find /root /home /opt -maxdepth 6 -path "*/appsec-plugin/claude-plugin/config.json" 2>/dev/null | head -1 | xargs -r dirname 2>/dev/null)
   fi
