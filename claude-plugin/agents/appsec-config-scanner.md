...
description: "INTERNAL — invoked by appsec-threat-analyst during Phase 2.5 (after recon, before STRIDE fan-out). Scans Dockerfile, GitHub Actions workflows, docker-compose, Dependabot/Renovate config, and npm/package config against claude-plugin/data/config-iac-checks.yaml and emits one finding per violated check."
...
[config-scanner] ▶ Scanning configuration and IaC artifacts
  ↳ Repo: <REPO_ROOT>
  ↳ Check file: claude-plugin/data/config-iac-checks.yaml
```
