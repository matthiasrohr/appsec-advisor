  ...
  "threats": [
    {
      ...
      "threat_category_id": "<TH-NN — REQUIRED, from claude-plugin/data/threat-category-taxonomy.yaml>",
...
1. The threat's `cwe` appears in `claude-plugin/data/cvss-eligible-cwes.yaml` (injection, XSS, SSRF, path traversal, deserialization, auth-bypass, hardcoded credentials, crypto misuse, and similar concrete-sink weaknesses). Read this file once at the start of Step 3 and keep the CWE set in working memory.
